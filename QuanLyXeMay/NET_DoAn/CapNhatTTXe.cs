...
namespace NET_DoAn
{
    public partial class CapNhatTTXe : Form
    {
        public CapNhatTTXe()
        {
            InitializeComponent();
        ...
        private void btxemloaixe_Click(object sender, EventArgs e)
        {
            Xemloaixe fr = new Xemloaixe();
            fr.Show();
            this.Hide();
        }
        ...
        private void btSua_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Xe x = db.Xes.Single(s => s.MaXe == txtMaxe.Text);
                    x.MaXe = txtMaxe.Text;
                    x.TenXe = txtTenxe.Text;
                    x.Maloaixe = cbMaloaixe.Text;
                    x.MaNCC = cbMaNCC.Text;
            ...
            if (MessageBox.Show("Bạn có muốn xóa không?", "Thông báo", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                for (int i = 0; i < dgvDsxe.SelectedRows.Count; i++)
                {
                    ...
                    var qr = from d in db.Xes
                             where d.MaXe == st
                             select d;
                    db.Xes.DeleteOnSubmit(qr.FirstOrDefault());
                }
                db.SubmitChanges();
        ...
        private void dgvDsxe_Click(object sender, EventArgs e)
        {
            txtMaxe.Enabled = false;
            if (btThem.Text == "Lưu")
            {
        ...
        private void CapNhatTTXe_Load(object sender, EventArgs e)
        {
            vohieuhoa(false);
            Loaddl();
            cbMaloaixe.DataSource = db.Xes.Select(s => s.Maloaixe).Distinct().ToList();
            cbMaNCC.DataSource = db.Xes.Select(s => s.MaNCC).Distinct().ToList();
        }
    }
}
