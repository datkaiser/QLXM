...
namespace NET_DoAn
{
    public partial class CapNhatTTNhanVien : Form
    {
        ...
        private void CapnhatTTNV_Load(object sender, EventArgs e)
        {
            LoadNV();
            vohieuhoa(false);
        }
        ...
        private void btSua_Click_1(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    NhanVien nv = db.NhanViens.Single(s => s.MaNV == txtManv.Text);
                    nv.MaNV = txtManv.Text;
                    nv.TenNV = txthoten.Text;
                    nv.GioiTinh = txtgt.Text;
                    nv.Ngaysinh = Convert.ToDateTime(txtNs.Text);
            ...
            if (MessageBox.Show("Bạn có muốn xóa không?", "Thông báo", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                for (int i = 0; i < dgvNV.SelectedRows.Count; i++)
                {
                    ...
                    var qr = from d in db.NhanViens
                             where d.MaNV == st
                             select d;
                    db.NhanViens.DeleteOnSubmit(qr.FirstOrDefault());
                }
                db.SubmitChanges();
